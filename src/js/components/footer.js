...
async function handleSpotifyError(response, actionName) {
  ...
  if (response.status === 403) {
    alert(`${actionName} 실패: Spotify 권한(scope) 또는 Premium 계정을 확인해주세요.`);
  } else {
    alert(`${actionName} 실패`);
  }
...
async function searchTrackFromBackend(track) {
  ...
  try {
    const response = await fetch(`${API_BASE_URL}/api/home/search?keyword=${encodeURIComponent(keyword)}`, {
      method: 'GET',
      credentials: 'include',
    });
    ...
    return {
      ...
      artist: firstTrack.artist || firstTrack.description || firstTrack.artistName || track.artist,
      cover: firstTrack.cover || firstTrack.imageUrl || firstTrack.image || track.cover,
    };
  } catch (error) {
...
function setFooterEmptyState(isEmpty) {
  ...
  const disabledButtonIds = ['shuffleButton', 'prevButton', 'nextButton', 'repeatButton', 'queueButton'];
...
async function playTestTrack() {
  ...
  const response = await spotifyRequest(`https://api.spotify.com/v1/me/player/play?device_id=${spotifyDeviceId}`, {
    method: 'PUT',
    body: JSON.stringify({
      uris: testTrackUris,
    }),
...
function updatePlayButtonIcon() {
  ...
  playButtonIcon.src = isPlaying ? '/assets/icon/Pause_XS.svg' : '/assets/icon/Play_XS.svg';
}
...
function updateShuffleButtonUI() {
  ...
  shuffleButton.setAttribute('aria-label', isShuffleOn ? '셔플 켜짐' : '셔플 꺼짐');
}
...
function updateCurrentTrackUI(track) {
  ...
  if (currentArtist) {
    currentArtist.textContent = track.artists?.map((artist) => artist.name).join(', ') || '';
  }
}
...
function updateProgressUI(position, duration) {
  ...
  if (progressBar) {
    const progress = currentDuration ? (currentPosition / currentDuration) * 100 : 0;
    ...
    const currentHTML = currentlyPlaying
      ? `
        <div class="player-popover__current">
          ...
          <span>
            ${escapeHTML(currentlyPlaying.artists?.map((artist) => artist.name).join(', ') || '-')}
          </span>
        </div>
      `
      : `
      ...
      devices.length > 0
        ? `
          <ul class="player-popover__list">
            ${devices
              .map((device) => {
                const activeText = device.is_active ? '현재 사용 중' : '대기 중';
                const volumeText =
                  typeof device.volume_percent === 'number' ? `${device.volume_percent}%` : '볼륨 정보 없음';

                return `
                  <li class="player-popover__item">
                    <span class="player-popover__device-dot ${device.is_active ? 'is-active' : ''}"></span>
